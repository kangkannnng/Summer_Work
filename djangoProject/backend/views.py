...
def hello(request):
    return JsonResponse({"status": 0, "message": "This is a message from Django"})  # 返回json数据类型
...
#             array1[int(similarlines[an2 + i][0])][int(similarlines[an2 + i][3])] += (
#                     int(similarlines[an2 + i][2]) - int(similarlines[an2 + i][1]))
#             array2[int(similarlines[an2 + i][0])][int(similarlines[an2 + i][3])] += (
#                     int(similarlines[an2 + i][5]) - int(similarlines[an2 + i][4]))
#             array1[int(similarlines[an2 + i][3])][int(similarlines[an2 + i][0])] += (
#                     int(similarlines[an2 + i][5]) - int(similarlines[an2 + i][4]))
#             array2[int(similarlines[an2 + i][3])][int(similarlines[an2 + i][0])] += (
#                     int(similarlines[an2 + i][2]) - int(similarlines[an2 + i][1]))
#         # 对两个二维数组进行赋值
...
#         for i in range(0, line1):
#             if array2[name][i] != 0:
#                 sortedlist.append(array1[name][i] + 1 / array2[name][i])
#             else:
